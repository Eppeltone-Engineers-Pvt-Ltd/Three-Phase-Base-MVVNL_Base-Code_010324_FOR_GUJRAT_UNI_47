...
// For Kerala, details of last and second last restored tampers are required
...
void check_clearTamper(uint16_t *ptr, uint32_t flagmask, uint16_t persistence_value, uint8_t flagBit)
{
  if((mytmprdata.tamperEvent_image&flagmask)!= 0)
  {//event has gone
    *ptr=*ptr+1;
//    if(*ptr >= TAMPER_PERSISTENCE_OFF)
    if(*ptr >= persistence_value)
    {// record the tamper
      *ptr = 0;
...
// Voltage Missing - to be checked first
	...
	{/* no two voltages are greater than 350 - one might be but not two of them*/
		...
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_R], computeMask(MISSING_POTENTIAL_R),TAMPER_PERSISTENCE_ON, 0, 0);
			misspotoccurencectr++;
		}
		
//		if (v[LINE_PHASE_R] > 1790)
		if (v[LINE_PHASE_R] > 970)
		{
	    check_clearTamper(&missingPotentialCtr[LINE_PHASE_R], computeMask(MISSING_POTENTIAL_R),TAMPER_PERSISTENCE_OFF, 0);
		}

		if ((v[LINE_PHASE_Y] < 960)&&(i[LINE_PHASE_Y]>95))
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_Y], computeMask(MISSING_POTENTIAL_Y),TAMPER_PERSISTENCE_ON, 1, 0);
			misspotoccurencectr++;
		}
		
//		if (v[LINE_PHASE_Y] > 1790)
		if (v[LINE_PHASE_Y] > 970)
		{
	    check_clearTamper(&missingPotentialCtr[LINE_PHASE_Y], computeMask(MISSING_POTENTIAL_Y),TAMPER_PERSISTENCE_OFF, 1);
		}
		
		if ((v[LINE_PHASE_B] < 960)&&(i[LINE_PHASE_B]>95))
		{
	    check_setTamper(&missingPotentialCtr[LINE_PHASE_B], computeMask(MISSING_POTENTIAL_B),TAMPER_PERSISTENCE_ON,2,0);
			misspotoccurencectr++;
		}
		
//		if (v[LINE_PHASE_B] > 1790)
		if (v[LINE_PHASE_B] > 970)
		{
	    check_clearTamper(&missingPotentialCtr[LINE_PHASE_B], computeMask(MISSING_POTENTIAL_B),TAMPER_PERSISTENCE_OFF,2);
		}
...
// after pot missing - check neutral disturbance - fault_code = 9	
	...
	{
    check_setTamper(&neutralDisturbanceCtr, computeMask(NEUTRAL_DISTURBANCE),TAMPER_PERSISTENCE_ON,9,0);
#ifdef ND_POWER_ON_ACTUALS		
	...
	{// here phasesGT300 can be 0,1, or 3(Really?)
		...
		{
    	check_clearTamper(&neutralDisturbanceCtr, computeMask(NEUTRAL_DISTURBANCE),TAMPER_PERSISTENCE_OFF,9);
...
//			if((((v[LINE_PHASE_R]<1800)&&(v[LINE_PHASE_R]>960))||((v[LINE_PHASE_Y]<1800)&&(v[LINE_PHASE_Y]>960))||((v[LINE_PHASE_B]<1800)&&(v[LINE_PHASE_B]>960)))&&((i[LINE_PHASE_R]>95)||(i[LINE_PHASE_Y]>95)||(i[LINE_PHASE_B]>95)))
			...
			{
		    check_setTamper(&lowVoltageCtr, computeMask(UNDER_VOLTAGE),TAMPER_PERSISTENCE_ON,99,0);	// 99 means no fault code
				g_TamperStatus|=UNDERVOLT_MASK;
			}
			
			if((v[LINE_PHASE_R]>1799)&&(v[LINE_PHASE_Y]>1799)&&(v[LINE_PHASE_B]>1799))
			{
		    check_clearTamper(&lowVoltageCtr, computeMask(UNDER_VOLTAGE),TAMPER_PERSISTENCE_OFF,99);
				g_TamperStatus&=~UNDERVOLT_MASK;
			}
		}
		...
		{
	    check_setTamper(&overVoltageCtr, computeMask(OVER_VOLTAGE),TAMPER_PERSISTENCE_ON,99,0);
			g_TamperStatus|=OVERVOLT_MASK;
		}
		
		if(phasesGT180LT276==3)
		{
	    check_clearTamper(&overVoltageCtr, computeMask(OVER_VOLTAGE),TAMPER_PERSISTENCE_OFF,99);
			g_TamperStatus&=~OVERVOLT_MASK;
		}
...
//		if((phasesGT144LT276==3)&&((g_TamperStatus&MISSPOT_MASK)==0)&&((g_TamperStatus&UNDERVOLT_MASK)==0)&&((g_TamperStatus&OVERVOLT_MASK)==0))
		if(phasesGT144LT276==3)
		{// all three phases are normal
			if(currentsGT95>=1)
			{
				if((maxValue-minValue)>(maxValue/10))
				{
			    check_setTamper(&voltageUnbalanceCtr, computeMask(VOLTAGE_UNBALANCED),TAMPER_PERSISTENCE_ON,3,0);
				}
			}
			
			if((maxValue-minValue)<(maxValue/10))
		    check_clearTamper(&voltageUnbalanceCtr, computeMask(VOLTAGE_UNBALANCED),TAMPER_PERSISTENCE_OFF,3);
		}
		else
		{
...
//#else
	  if (temp_power < 0){
	    check_setTamper(&ctReverseCtr[LINE_PHASE_R], computeMask(CT_REVERSED_R),TAMPER_PERSISTENCE_ON,6,0);
			currentreversectr++;}
	  else
	    check_clearTamper(&ctReverseCtr[LINE_PHASE_R], computeMask(CT_REVERSED_R),TAMPER_PERSISTENCE_OFF,6);
	}
//#endif
...
//#else
	  if (temp_power < 0){
	    check_setTamper(&ctReverseCtr[LINE_PHASE_Y], computeMask(CT_REVERSED_Y),TAMPER_PERSISTENCE_ON,7,0);
			currentreversectr++;}
	  else
	    check_clearTamper(&ctReverseCtr[LINE_PHASE_Y], computeMask(CT_REVERSED_Y),TAMPER_PERSISTENCE_OFF,7);
//#endif
	}
...
//#else
	  if (temp_power < 0){
	    check_setTamper(&ctReverseCtr[LINE_PHASE_B], computeMask(CT_REVERSED_B),TAMPER_PERSISTENCE_ON,8,0);
			currentreversectr++;}
		else		
	    check_clearTamper(&ctReverseCtr[LINE_PHASE_B], computeMask(CT_REVERSED_B),TAMPER_PERSISTENCE_OFF,8);
//#endif
	}
...
//	if((phasesGT180LT276==3)&&((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
//#ifdef BIDIR_METER
	if((phasesGT180LT276==3)&&(mixedPowers==0)&&((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
//#else
//	if((phasesGT180LT276==3)&&((g_TamperStatus&REVERSE_MASK)==0)&&((g_TamperStatus&HALLSENS_MASK)==0)&&((g_TamperStatus&ACMAG_MASK)==0))
//#endif
	{
		if(currentsGT95>=1)
		{
		  ...
		  if(i[LINE_PHASE_R] < (maxValue/50))
			{
			  check_setTamper(&ctOpenCtr[LINE_PHASE_R], computeMask(CT_OPEN_R),TAMPER_PERSISTENCE_ON,11,0);
				ct_open_ctr++;
			}
		  else
		    check_clearTamper(&ctOpenCtr[LINE_PHASE_R], computeMask(CT_OPEN_R),TAMPER_PERSISTENCE_OFF,11);
				
		  if(i[LINE_PHASE_Y] < (maxValue/50))
			{
		    check_setTamper(&ctOpenCtr[LINE_PHASE_Y], computeMask(CT_OPEN_Y),TAMPER_PERSISTENCE_ON,12,0);
				ct_open_ctr++;
			}
		  else
		    check_clearTamper(&ctOpenCtr[LINE_PHASE_Y], computeMask(CT_OPEN_Y),TAMPER_PERSISTENCE_OFF,12);
				
		  if(i[LINE_PHASE_B] < (maxValue/50))
			{
				check_setTamper(&ctOpenCtr[LINE_PHASE_B], computeMask(CT_OPEN_B),TAMPER_PERSISTENCE_ON,13,0);
				ct_open_ctr++;
			}
		  else
		    check_clearTamper(&ctOpenCtr[LINE_PHASE_B], computeMask(CT_OPEN_B),TAMPER_PERSISTENCE_OFF,13);
		}
		...
		{		
		  if((ineutralDiff > 90)&&(i[LINE_PHASE_R]>18)&&(i[LINE_PHASE_Y]>18)&&(i[LINE_PHASE_B]>18)&&(1))
			{
		    check_setTamper(&ctBypassCtr, computeMask(CT_BYPASS),TAMPER_PERSISTENCE_ON,4,0);
				g_TamperStatus|=BYPASS_MASK;
			}
			else
			{
		    check_clearTamper(&ctBypassCtr, computeMask(CT_BYPASS),TAMPER_PERSISTENCE_OFF,4);
				g_TamperStatus&=~BYPASS_MASK;
			}	
		}
		...
		{// Check current unbalance condition			if((i[LINE_PHASE_R]>95)&&(i[LINE_PHASE_Y]>95)&&(i[LINE_PHASE_B]>95))
			{
				...
				{
			    check_setTamper(&currentUnbalanceCtr, computeMask(CURRENT_UNBALANCED),TAMPER_PERSISTENCE_ON,5,0);
				}
				else
			    check_clearTamper(&currentUnbalanceCtr, computeMask(CURRENT_UNBALANCED),TAMPER_PERSISTENCE_OFF,5);
			}
		}	
...
*/			
  ...
  // Check hall sensor condition
	...
	{
    check_setTamper(&magnetCtr, computeMask(MAGNET),MAGNET_TAMPER_PERSISTENCE_ON,10,0);
		g_TamperStatus|=HALLSENS_MASK;
	}
	else
	{
    check_clearTamper(&magnetCtr, computeMask(MAGNET),MAGNET_TAMPER_PERSISTENCE_OFF,10);
		g_TamperStatus&=~HALLSENS_MASK;
	}
