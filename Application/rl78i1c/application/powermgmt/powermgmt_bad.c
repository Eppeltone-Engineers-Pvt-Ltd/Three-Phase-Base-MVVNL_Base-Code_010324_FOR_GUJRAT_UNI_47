...
{// upon reset the current mode is mode2
  ...
  {// the variable g_powermgmt_prev_main_mode contains the mode before entering MODE2
...
// we come to mode2 either when power has failed or power has returned
// hence we could come here from MODE1, MODE3 or MODE4
// every time we exit from any of the above there modes the mode1_counter is made 0

    case POWERMGMT_MODE2:
			if((g_powermgmt_prev_main_mode==POWERMGMT_MODE3)||(g_powermgmt_prev_main_mode==POWERMGMT_MODE4))
      ...
      {// VDD pin has not yet gone to upper limit or VDD is falling
				...
				// here g_powermgmt_flag.is_ac_low = 1 
        if (mode1_counter < 1000)	// for testing - earlier 1000
