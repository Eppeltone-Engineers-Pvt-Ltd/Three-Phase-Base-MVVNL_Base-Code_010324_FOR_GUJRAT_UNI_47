...
powermgmt_int_flag_t    g_powermgmt_flag = {0, 0, 0, 0, 0, 0, 0, 0};    /* Flag to control interrupt in powermgmt */

//static void POWERMGMT_DisplayLCDParams(float32_t kwh);
static uint8_t POWERMGMT_ProcessMode1(void);
static uint8_t POWERMGMT_ProcessMode2(void);
static uint8_t POWERMGMT_ProcessMode3(void);
static uint8_t POWERMGMT_ProcessMode3_Pseudo(void);
static uint8_t POWERMGMT_ProcessMode4(void);
static uint8_t POWERMGMT_ProcessMode5(void);
static uint8_t POWERMGMT_ProcessMode7(void);

static void POWERMGMT_DoAfterStop(void);
static void POWERMGMT_DoBeforeStop(void);
static void POWERMGMT_ACLow_Action(void);
...
// new code start		
		...
		CSS=0;
		while (CLS!=0);
// CPU is now running on VDD and main system clock		
// new code end		
...
// The DI and EI statements enclosing the block of statements produce no change 
...
//		POWERMGMT_ProcessMode3_Pseudo();	// This is written to simulate a mode3 before entering Mode 1
// To see the magic , if this will cause the communication to start working		
	
    POWERMGMT_DoAfterStop();

//		record_power_fail_event();	// both fail and return - shifted below		
	LCD_ClearAll();

	
// make a copy of ctldata time stamp
	if((capturedRTC.year==0)&&(capturedRTC.month==0)&&(capturedRTC.date==0)&&(capturedRTC.hour==0)&&(capturedRTC.minute==0))
	{
	...
	{
	  lastTime.minute=capturedRTC.minute;
	  lastTime.hour=capturedRTC.hour;
	  lastTime.date=capturedRTC.date;
	  lastTime.month=capturedRTC.month;
	  lastTime.year=capturedRTC.year;
	}

		readRTC();

		do_initial_things(); // required to be done upon return of power
		
//		init_done=1;	// shifted below
...
//		asendbare((uint8_t *)"Ent Mode 1\r\n");  		
    ...
    if (EM_GetSystemState() != SYSTEM_STATE_RUNNING)
    {
        if (EM_Start() != EM_OK)
        {
//            POWERMGMT_DisplayLCDError(1.01);
...
//  	LCD_DisplayDigit(1, 1);
		...
		{
			if(((rtc.hour*60+rtc.minute)-(lastTime.hour*60+lastTime.minute))>=5)
				record_power_fail_event();	// both fail and return - shifted below		
		}
		else
		{// here date or month or year are different
			record_power_fail_event();	// both fail and return - shifted below		
		}
// it is possible that record_power_fail_event may return without writing the power return record
// if the EM has not returned voltage values
//		clear_tamperEvent_image(POWER_EVENT);	// record the power recovery tamper
...
/*		
		if(g_powermgmt_previous_mode==POWERMGMT_MODE3)
		{
//        R_LCD_PowerOff();
        // Switch CPU back to fMAIN,
//        R_CGC_FclkUseFih();
		}
*/    
    return 0;
...
//						P3|=0x80;	//E2ROM_SUPPLY_ON - This remains on - SCL, SDA and WP lines are made input
	R_RTC_Set_ConstPeriodInterruptOn(PES_1_SEC);  /* Turn On one second interrupt to increment disp_timer */
	P6 =0x05;	// WP=1,SDA=0,SCL=1
	PM6 = 0xFA;	// 1111 1010 - WP(Out), SDA(Inp), SCL(Out)
  ...
  switch (g_powermgmt_current_mode)
  {
    ...
    case POWERMGMT_MODE3:
      ...
      {// here there is no power and we are in mode three
				if((disp_state==10)&&(sleepCommMode==0))
				{
    ...
    case POWERMGMT_MODE4:
      ...
      {// Switch to transition mode and log - ac power has returned
        ...
        mode1_counter = 0;
			...
			else if(g_powermgmt_flag.is_key_int==1)
			{// here switch has been pressed - new mode
				new_mode = POWERMGMT_MODE3;
				g_powermgmt_flag.is_key_int=0;
			}
			else if(g_powermgmt_flag.is_cover_int==1)
...
{
#ifdef POWER_MANAGEMENT_ENABLE
    if(LVDVRTCF == 0)    //EXLVD VOLTAGE > DETECTION THRESHOLD
    {
