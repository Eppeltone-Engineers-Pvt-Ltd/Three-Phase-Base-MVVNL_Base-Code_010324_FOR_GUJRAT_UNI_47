// em_display_gujarat_bidir_ltct.c
// Driver layer 
...
#ifdef BIDIR_METER
	#define NO_OF_SUB_DISPLAY_STATES 6	// THIS BIDIR_METER has 6
#else
...
//	disp_autoscroll_interval=5;
	...
	{
#ifdef BIDIR_METER		
		if((g_display_mode==0)&&(disp_state==45))
#else
...
void displayRepeatParameters(void)
{
  ...
  LCD_ClearAll();
    ...
    case 2:		//Total Cumulative Import kwh	-	Solar hours script id 1, (scriptid-1)=0
			...
			{
				tempTodKwh=myenergydata.zone_kwh; // myenergydata.zone_kwh
			}
...
//			display_long_one_dp(tempTodKwh/100);
    ...
    case 3:		//Total Cumulative Export kwh	-	Solar hours script id 1, (scriptid-1)=0
			tempchar=0;	// this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+tempchar*ZONE_PTR_INC+32, (uint8_t*)&tempTodKwh, sizeof(tempTodKwh));
			if(tempchar==presentZone)
			{
				tempTodKwh=myenergydata.zone_kwh_export; // myenergydata.zone_kwh
			}
...
// Cum Import KWH Solar hours
    case 22:		//Import kwh	-	Solar hours script id 1, (scriptid-1)=0
			tempchar=0;	// this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+tempchar*ZONE_PTR_INC, (uint8_t*)&tempTodKwh, sizeof(tempTodKwh));
			if(tempchar==presentZone)
			{
...
// Cum Import KWH Rest hours
    case 23:			// Import kwh - Rest Hours script id 2, (scriptid-1)=1
...
// Cum Export KWH Solar hours
    case 24:		//Export kwh	-	Solar hours script id 1, (scriptid-1)=0
			tempchar=0;	// this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+tempchar*ZONE_PTR_INC+32, (uint8_t*)&tempTodKwh, sizeof(tempTodKwh));
			if(tempchar==presentZone)
			{
...
// Cum Export KWH Rest hours
    case 25:			// Export kwh - Non solar Hours script id 2, (scriptid-1)=1
			tempchar=1; // this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+tempchar*ZONE_PTR_INC+32, (uint8_t*)&tempTodKwh, sizeof(tempTodKwh));
			if(tempchar==presentZone)
			{
				tempTodKwh=myenergydata.zone_kwh_export;
			}
...
// MD Import kw for present Billing Period - Solar Hours
    case 27:		//MD Import kw	-	Solar hours script id 1, (scriptid-1)=0
			tempchar=0;	// this is now (scriptid -1)
...
// MD Import kw for present Billing Period - Rest Hours
    case 28:		//MD Import kw	-	script id 2, (scriptid-1)=1
			tempchar=1;	// this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+12+tempchar*ZONE_PTR_INC, (uint8_t*)&tempTodMD, sizeof(tempTodMD));
			displayPrepare();
			display_long_three_dp(tempTodMD);
...
// MD Import kw for last Billing Period - this is now 4 bytes
    case 29:		// L1 - MD
      ...
      EPR_Read(BILLDATA_BASE_ADDRESS+(tmonth-1)*BILL_PTR_INC+80,(uint8_t*)&tempLong,4);
			displayPrepare();
      display_long_three_dp(tempLong);
...
// MD Import kw for last Billing Period - Solar Hours - 4bytes
    case 30:		//MD Import kw	-	Solar hours script id 1, (scriptid-1)=0
		// this information is to be picked up from Billing area in e2rom
      tmonth = decrementMonth(1);
			tempchar=0;	// this is now (scriptid-1)
      EPR_Read(BILLDATA_BASE_ADDRESS+(tmonth-1)*BILL_PTR_INC + 96 + (tempchar*4),(uint8_t*)&tempLong1,sizeof(tempLong1));
			displayPrepare();
...
// MD Import kw for last Billing Period - Rest Hours
...
// MD Export kw for present Billing Period
    case 32:		// Current MD Export in kW
			displayPrepare();
...
// MD Export kw for present Billing Period - Solar Hours
    case 33:		//MD Export kw	-	Peak hours script id 1, (scriptid-1)=0
			tempchar=0;
...
// MD Export kw for present Billing Period - Rest Hours
    case 34:		//MD Export kw	- Night hours	script id 2, (scriptid-1)=1
			tempchar=1;	// this is now (scriptid-1)
			EPR_Read(TOD_CURRENT_BASE_ADDRESS+40+tempchar*ZONE_PTR_INC, (uint8_t*)&tempTodMD, sizeof(tempTodMD));
			displayPrepare();
			display_long_three_dp(tempTodMD);
...
// MD Export kw for last Billing Period
    case 35:		// L1 - MD Export 
      tempchar = 1;
      tmonth = decrementMonth(tempchar);
      EPR_Read(BILLDATA_BASE_ADDRESS+(tmonth-1)*BILL_PTR_INC+372,(uint8_t*)&tempLong,sizeof(tempLong));
			displayPrepare();
      display_long_three_dp(tempLong);
		  LCD_DisplaySpSign(S_MD);
		  LCD_DisplaySpSign(S_BILL);
...
// MD Export kw for last Billing Period - Solar Hours
    case 36:		//MD Export kw	-	Solar hours , scriptid 1, (scriptid-1)=0
		// this information is to be picked up from Billing area in e2rom
      tmonth = decrementMonth(1);
			tempchar=0;	// this is now (scriptid-1)
      EPR_Read(BILLDATA_BASE_ADDRESS+(tmonth-1)*BILL_PTR_INC + 388 + (tempchar*4),(uint8_t*)&tempLong1,sizeof(tempLong1));
			displayPrepare();
			display_long_three_dp(tempLong1);
			LCD_DisplaykWSign();
		  LCD_DisplaySpSign(S_BILL);
...
// MD Export kw for last Billing Period - Rest Hours
    case 37:		//MD Export kw	-	Rest hours scriptid 2, (scriptid-1)=1
		// this information is to be picked up from Billing area in e2rom
      tmonth = decrementMonth(1);
			tempchar=1;	// this is now (scriptid-1)
      EPR_Read(BILLDATA_BASE_ADDRESS+(tmonth-1)*BILL_PTR_INC + 388 + (tempchar*4),(uint8_t*)&tempLong1,sizeof(tempLong1));
