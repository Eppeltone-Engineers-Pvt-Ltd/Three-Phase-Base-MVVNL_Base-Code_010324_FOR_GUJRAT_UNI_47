...
#include "r_dlms_user_interface.h"
...
extern date_time_t g_Class04_mdCaptureTime;


//Unsigned16 passive_script_id[8];
/* Functions */
...
*                 : Unsigned8 * value_buf    : Buffer that store value needed to be encoded
...
)
{
	...
	switch (attr_type)
	{
		...
		case ATTR_TYPE_UTF8_STRING:
			if (buf_len >= 2)
			{// All these attribute types have their length stored in the following byte
			// the caller of this function also provides the length which is used as a sanity check
			// subsequently those many bytes from the buffer are transferred to the value_buf
				// TODO, Check here
				buf++;					// Ignore the type - advance pointer by 1
				// Get the item length 
				temp_len = BERLengthDecode(buf, &item_length);	// This function is hidden - TOP SECRET.
				buf += temp_len;
				...
				// Transfer value from source buffer to destination buffer
...
/******************************************************************************
* Function Name : R OBIS_DistributeRegisterClass
* Interface     : Unsigned8 R OBIS_DistributeRegisterClass(
*               :     Unsigned16 master_index,
*               :     Unsigned16 child_index,
*               :     req_type_t req_type,
*               :     service_type_t service_type,
...
)
{
	/* Result of response, def. is not Matched */
	Unsigned8				response_result = TYPE_UNMATCH;
	Unsigned8 childRecordIndex;
	
	class04_child_record_t	*p_child_record;
	class04_child_record_t	*p_child_record_base;	// this points to the first record of the table

	p_child_record = (class04_child_record_t *)(
		R_OBIS_FindChildRecordByIndex(master_index, child_index)
	);

	p_child_record_base = (class04_child_record_t *)(
		R_OBIS_FindChildRecordByIndex(master_index, 0)
	);
	...
	if (req_type == REQ_TYPE_GET || req_type == REQ_TYPE_SET)
	{
		switch (p_descriptor->Attr_ID)
		{
			...
			case 5:	/* capture_time */
				...
				/* distribute the buffer */
// since we are using a common buffer for capture time - hence we need to fill it right now
// the g_ChildTableClass04 has 4 entries - we need to find out what is the entry number whose data is sought
// we can do that by subtracting the present p_child_record pointer from the base address and divide by the 
// size of each child record
//				childRecordIndex = (Unsigned8)(((int32_t)p_child_record - (int32_t)p_child_record_base)/((int32_t)(sizeof(class04_child_record_t))));
				childRecordIndex = ((int32_t)p_child_record - (int32_t)p_child_record_base)/((int32_t)(sizeof(class04_child_record_t)));

				switch(childRecordIndex)
				{
					case 0:
...
/******************************************************************************
* Function Name : R OBIS_DistributeProfileGenericClass
* Interface     : Unsigned8 R OBIS_DistributeProfileGenericClass(
*               :     Unsigned16 master_index,
*               :     Unsigned16 child_index,
*               :     req_type_t req_type,
*               :     service_type_t service_type,
...
)
{
	...
	Unsigned8 tbuf[8];	// added on 20/Jul/2023 to avoid storing OBIS codes for day_action_t etc
	Unsigned8* ptr;
	int8_t match_result;
	...
	if (req_type == REQ_TYPE_GET || req_type == REQ_TYPE_SET)
	{
		switch (p_descriptor->Attr_ID)
		{
			...
			case 1: /* logical_name */
				if (req_type == REQ_TYPE_GET)
				{
					*p_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,p_descriptor->Instance_ID,6);
					response_result = DATA_ACCESS_RESULT_SUCCESS;
				}
				else
				{}
...
// The original function is now uncommented as it was causing some problemin CTT			
...
/*
				if (req_type == REQ_TYPE_GET)
				{
						temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,p_child_record->p_calendar_name_active,*p_child_record->p_calendar_name_active_len);
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;
						response_result = DATA_ACCESS_RESULT_SUCCESS;
				}
...
*/				
			...
			case 7:	// season_profile_passive 
				if (req_type == REQ_TYPE_GET)
				{
					...
					*p_out_buf++ = profile_nr;	// this number happens to be 2, as we have two seasons defined
					*p_out_len   = 2;
					// season_profile_t
					for (i = 0; i < profile_nr; i ++)
					{
						...
						*p_out_buf++ = ATTR_TYPE_STRUCTURE;								// Structure type
						*p_out_buf++ = 3;												// 3 elements season_name, season_start, week_profile name
						*p_out_len   += 2;

						// p_season_profile_name Encode
						temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,p_child_seanson_profile->p_season_profile_name,p_child_seanson_profile->season_profile_name_len);
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;

						// season_start Encode
						temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,(Unsigned8 *)p_child_seanson_profile->p_season_start,sizeof(date_time_t));
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;

						// week_profile_name Encode
						temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,p_child_seanson_profile->p_week_profile_name,p_child_seanson_profile->week_profile_name_len);
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;
					}
					response_result = DATA_ACCESS_RESULT_SUCCESS;
				...
				else if (req_type == REQ_TYPE_SET)
				{
					...
					for (i = 0; i < profile_nr; i ++)
					{
						...
						if (*pdata++ != 3)	// 3 elements  - season name, start time and week profile name
							return response_result;

						// p_season_profile_name Decode - the assumption is that the client is going to send a season name whose length must not be > season_profile_name_len
						temp_out_len = R_OBIS_DecodeAttribute((Unsigned8 *)p_child_seanson_profile->p_season_profile_name,p_child_seanson_profile->season_profile_name_len,ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						pdata += temp_out_len;

						// season_start Decode
						if (R_OBIS_IsValidDateTime(pdata)== 0) // Invalid
							break;
						temp_out_len = R_OBIS_DecodeAttribute((Unsigned8 *)p_child_seanson_profile->p_season_start,sizeof(date_time_t),ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						pdata += temp_out_len;

						// week_profile_name decode
						temp_out_len = R_OBIS_DecodeAttribute((Unsigned8 *)p_child_seanson_profile->p_week_profile_name,p_child_seanson_profile->week_profile_name_len,ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						pdata += temp_out_len;
					}
					response_result = DATA_ACCESS_RESULT_SUCCESS;
					write_passive_tod();	// added here on 02/Oct/23
				}
				else{}
				break;
			...
			case 8:	// week_profile_passsive
					...
					*p_out_len   = 2;
					...
					for (i = 0; i < profile_nr; i ++)
					{
						...
						*p_out_len   += 2;
						...
						// week_profile_name encode
						temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,p_child_week_profile->p_week_profile_name,p_child_week_profile->week_profile_name_len);
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;
				...
				else if (req_type == REQ_TYPE_SET)
				{
					...
					if (active_choice == 1)
						profile_nr = p_child_record->nr_week_profile_active;
					else
						profile_nr = p_child_record->nr_week_profile_passive;
					...
					for (i = 0; i < profile_nr; i ++)
					{
						...
						// week_profile_name decode 
						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)p_child_week_profile->p_week_profile_name,p_child_week_profile->week_profile_name_len,ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						pdata += temp_out_len;
						...
						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Tuesday = tbuf[0];
						pdata += temp_out_len;

						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Wednesday = tbuf[0];
						pdata += temp_out_len;
						
						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Thursday = tbuf[0];
						pdata += temp_out_len;

						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Friday = tbuf[0];
						pdata += temp_out_len;

						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Saturday = tbuf[0];
						pdata += temp_out_len;

						temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,1,ATTR_TYPE_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
						p_child_week_profile->Sunday = tbuf[0];
						pdata += temp_out_len;
					}
					response_result = DATA_ACCESS_RESULT_SUCCESS;
					write_passive_tod();	// added here on 02/Oct/23
			...
			case 9:	// day_profile_passive
				if (req_type == REQ_TYPE_GET)
				{
					...
					*p_out_buf++ = ATTR_TYPE_ARRAY;								// Structure type
					if (active_choice == 1)
						profile_nr = p_child_record->nr_day_profile_active;		// No. elements - 4 in our case at present
					else
						profile_nr = p_child_record->nr_day_profile_passive;	// No. elements - 4 in our case at present
					...
					*p_out_len   = 2;
					...
					for (i = 0; i < profile_nr; i ++)
					{
						...
						*p_out_buf++ = ATTR_TYPE_STRUCTURE;							// Structure type
						*p_out_buf++ = 2;											// 2 elements are sent - the dayId and the day_profile action array having nr_day_schedule rows
						*p_out_len   += 2;
						...
						*p_out_buf++ = ATTR_TYPE_ARRAY;								// Structure type
						*p_out_buf++ = p_child_day_profile->nr_day_schedule;		// 4 profiles */
						*p_out_len   += 2;
						...
						for (j = 0; j < p_child_day_profile->nr_day_schedule; j ++)
						{
							...
							*p_out_buf++ = ATTR_TYPE_STRUCTURE;			// Structure type
							*p_out_buf++ = 3;												// 3 elements starttime, obiscode, scriptid
							*p_out_len   += 2;

							// start_time
//							ptr = (Unsigned8 *)p_child_day_profile_action->p_start_time;
							tbuf[0] = p_child_day_profile_action->p_start_time->hour;
							tbuf[1] = p_child_day_profile_action->p_start_time->minute;
							tbuf[2] = 0;
							tbuf[3] = 0xFF;	// TIME_HUNDREDTHS_NOT_SPECIFIED

							temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,(uint8_t *)tbuf,sizeof(time_t));
//							temp_out_len = R_OBIS_EncodeAttribute(p_out_buf,OBIS_SERVICE_DATA_BUFFER_LENGTH,ATTR_TYPE_OCTET_STRING,(uint8_t *)p_child_day_profile_action->p_start_time,sizeof(time_t));

							p_out_buf = p_out_buf + temp_out_len;
							*p_out_len += temp_out_len;

							// script_logical_name
// we can save 6*8=48 bytes for every day_profile action by not storing the OBIS code

							*p_out_buf++ =  ATTR_TYPE_OCTET_STRING;
							*p_out_buf++ =  6;
/*							
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[0];
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[1];
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[2];
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[3];
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[4];
							*p_out_buf++ =  (uint8_t)p_child_day_profile_action->script_logical_name[5];
*/							
							*p_out_buf++ =  (uint8_t)0;
							*p_out_buf++ =  (uint8_t)j;
							*p_out_buf++ =  (uint8_t)10;
							*p_out_buf++ =  (uint8_t)0;
							*p_out_buf++ =  (uint8_t)100;
							*p_out_buf++ =  (uint8_t)255;
							
							*p_out_len   += 8;
							
							// Script ID
							*p_out_buf++ = ATTR_TYPE_LONG_UNSIGNED;
							*p_out_buf++ = (uint8_t)(p_child_day_profile_action->script_selector >> 8);
							*p_out_buf++ = (uint8_t)(p_child_day_profile_action->script_selector  & 0xFF);
							*p_out_len   += 3;
						}
					}
					if (*p_out_len > OBIS_SERVICE_DATA_BUFFER_LENGTH)
				...
				else if (req_type == REQ_TYPE_SET)
				{
					...
					if (active_choice == 1)
						profile_nr = p_child_record->nr_day_profile_active;		// No. elements - 4 of them at present
					else
						profile_nr = p_child_record->nr_day_profile_passive;	// No. elements - 4 of them at present
					...
					for (i = 0; i < profile_nr; i ++)
					{
						...
						if (*pdata++ != 2)	// 2 elements - dayId and the day_profile_action arrays having nr_day_schedule rows
							return response_result;

						// day_id
						if (*pdata++ != ATTR_TYPE_UNSIGNED)
							return response_result;
							
						p_child_day_profile->day_id = (uint8_t)*pdata++;
						...
						if (*pdata++ != p_child_day_profile->nr_day_schedule)	// 8 elements at present
							return response_result;
						...
						for (j = 0; j < p_child_day_profile->nr_day_schedule; j ++)
						{
							...
							if (*pdata++ != 3)	// 3 elements - start time, obis code , scriptid */
								return response_result;

							// start_time
							
// get the data into tbuf							
							temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,sizeof(time_t),ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
							
// now transfer it to the appropriate two byte buffer
							p_child_day_profile_action->p_start_time->hour = tbuf[0];
							p_child_day_profile_action->p_start_time->minute = tbuf[1];
/*							
							temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)p_child_day_profile_action->p_start_time,sizeof(time_t),ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
*/
...
/*							
							temp_out_len = R_OBIS_DecodeAttribute(
										(uint8_t *)p_child_day_profile_action->script_logical_name,6,ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
*/
							temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)tbuf,6,ATTR_TYPE_OCTET_STRING,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
// just junk the tbuf - we dont store it																				
							pdata += temp_out_len;

							// Script ID 
							temp_out_len = R_OBIS_DecodeAttribute((uint8_t *)&p_child_day_profile_action->script_selector,2,ATTR_TYPE_LONG_UNSIGNED,pdata,OBIS_SERVICE_DATA_BUFFER_LENGTH);
							pdata += temp_out_len;
						}
				}
...
// we must save the updated passive start times in e2om here - ajay
					write_passive_tod();	// located in memoryOps.c - commented on 18/Jul/2023 - must be rewritten
...
/*
				if (req_type == REQ_TYPE_GET)
				{
						temp_out_len = R_OBIS_EncodeAttribute(
							...
							OBIS_SERVICE_DATA_BUFFER_LENGTH,								// Max length of g_ServiceBuffer
							ATTR_TYPE_OCTET_STRING,											// Type
							p_child_record->p_calendar_name_passive,					// Buffer
							*p_child_record->p_calendar_name_passive_len				// Length
						);
						p_out_buf = p_out_buf + temp_out_len;
						*p_out_len += temp_out_len;
				...
				{	// Do nothing, calendar_name_passive is read only
					// Do remember to change the access method of this element to read only
				}
*/				
				break;
			...
			case 10:	/* activate_passive_calendar_time  */
				...
				if (req_type == REQ_TYPE_SET)
				{// here we should check if the date is in the future
//----------------------------------------------------------------------	
					ConvertRTCDate(&current_time, &rtc);
					match_result = R_OBIS_CompareDate((Unsigned8 *)&g_Class20_ac_time, (Unsigned8 *)&current_time);
					if(match_result<0)
					...
					{// given date is today or in the future
						if(match_result==0)
						{// if today then check time
							match_result = R_OBIS_CompareTime((Unsigned8 *)&g_Class20_ac_time.hour, (Unsigned8 *)&current_time.hour);
							if(match_result<0)
							{// given time is in the past
								response_result=ACTION_RESULT_TYPE_UNMATCH;
								break;
							}
						}
...
//-------------------------------------------------------------------------					
					response_result= R_USER_Class20_Active_Passive_Time();
					class20_status=1;	// activation is pending
					write_passive_tod_activation_data(class20_status);	// located in memoryOps.c
					record_transaction_event(ACTIVITY_CALENDAR_TRANSACTION_MASK);
				...
				else if (req_type == REQ_TYPE_SET)
				{
					...
					for (i = 0; i < p_child_record->nr_execution_time; i ++)
					{
						...
						tt =(date_t *) p_child_exe_time->p_date;
						 bill_day = tt->day_of_month;
					}
//----------------------------------------------------------------------	
					ConvertRTCDate(&current_time, &rtc);
					match_result = R_OBIS_CompareDate((Unsigned8 *)&g_Class22_billdate, (Unsigned8 *)&current_time);
					if(match_result<0)
					{// given date is in the past - not acceptable
						response_result=ACTION_RESULT_TYPE_UNMATCH;
						break;
					...
					{// given date is today or in the future
						if(match_result==0)
						{// if today then check time
							match_result = R_OBIS_CompareTime((Unsigned8 *)&g_Class22_billtime, (Unsigned8 *)&current_time.hour);
							if(match_result<0)
							{// given time is in the past
								response_result=ACTION_RESULT_TYPE_UNMATCH;
								break;
...
// we should save the single action billing date and time in e2rom
					class22_status=1;
					write_single_action_activation_data(class22_status);	// located in memoryOps.c
