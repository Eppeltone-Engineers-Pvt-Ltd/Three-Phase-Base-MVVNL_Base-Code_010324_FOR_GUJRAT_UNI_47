...
const scaler_unit_t		g_Class07_BlockloadScalerBuffer[] =
{
	...
	{SCALAR_APF, PHY_UNIT_NONE			},	// Block Power Factor R-phase 
	{SCALAR_APF, PHY_UNIT_NONE			},	// Block Power Factor Y-phase 
	{SCALAR_APF, PHY_UNIT_NONE			},	// Block Power Factor B-phase 
#endif
...
const scaler_unit_t			g_Class07_BillingScalerBuffer[] =
{
	...
	{SCALAR_WH, PHY_UNIT_WATT_HOUR			},	// Cumulative Energy kWh TZ6 -export  
#ifndef SIXZONES	
	{SCALAR_WH, PHY_UNIT_WATT_HOUR			},	// Cumulative Energy kWh TZ7 -export  
	{SCALAR_WH, PHY_UNIT_WATT_HOUR			},	// Cumulative Energy kWh TZ8 -export
#endif	
	...
	{SCALAR_VAH, PHY_UNIT_VOLT_AMPERE_HOUR		},	// Cumulative Energy kVAh TZ6 -export
#ifndef SIXZONES		
	{SCALAR_VAH, PHY_UNIT_VOLT_AMPERE_HOUR		},	// Cumulative Energy kVAh TZ7 -export  
	{SCALAR_VAH, PHY_UNIT_VOLT_AMPERE_HOUR		},	// Cumulative Energy kVAh TZ8 -export 
#endif
	...
	{SCALAR_MD, PHY_UNIT_WATT			},	// MD kW for TZ6 -export 
#ifndef SIXZONES	
	{SCALAR_MD, PHY_UNIT_WATT			},	// MD kW for TZ7 -export 
	{SCALAR_MD, PHY_UNIT_WATT			},	// MD kW for TZ8 -export 
#endif	
	...
	{SCALAR_MD, PHY_UNIT_VOLT_AMPERE	},	// MD kVA TZ6 -export 
#ifndef SIXZONES	
	{SCALAR_MD, PHY_UNIT_VOLT_AMPERE	},	// MD kVA TZ7 -export 
	{SCALAR_MD, PHY_UNIT_VOLT_AMPERE	},	// MD kVA TZ8 -export
#endif	
};
#else
const scaler_unit_t			g_Class07_BillingScalerBuffer[] =
{
	...
	//{SCALAR_NULL, PHY_UNIT_NONE						},	// MD kVA time  
	{SCALAR_NULL, PHY_UNIT_MIN		},	// Billing power ON duration in minutes
#ifdef UNIDIR_TOD_HAS_KVARH_LAG
...
const scaler_unit_t		g_Class07_Event0ScalerBuffer[] =
{
	...
	{SCALAR_VOLTAGE, PHY_UNIT_VOLT			},	// Voltage  IR 
	{SCALAR_VOLTAGE, PHY_UNIT_VOLT			},	// Voltage  IY 
	{SCALAR_VOLTAGE, PHY_UNIT_VOLT			},	// Voltage  IB 
	{SCALAR_APF, PHY_UNIT_NONE			},	// Signed Power Factor R-phase 
	{SCALAR_APF, PHY_UNIT_NONE			},	// Signed Power Factor Y-phase 
	{SCALAR_APF, PHY_UNIT_NONE			},	// Signed Power Factor B-phase 
	{SCALAR_WH, PHY_UNIT_WATT_HOUR			},	// Cumulative Energy - kWh 
...
//If there is no overflow, then the oldest record index is 0
	...
	{// here we have an overflow condition
		search_index = (ctldata.survey_wrt_ptr - (int32_t)SURVEY_BASE_ADDRESS)/(int32_t)SURVEY_PTR_INC;	// oldest record
	}
	else
...
//	addr=(int32_t)SURVEY_BASE_ADDRESS + (int32_t)search_index*(int32_t)sizeof(surveydata_t);
	addr=(int32_t)SURVEY_BASE_ADDRESS + (int32_t)search_index*(int32_t)SURVEY_PTR_INC;
...
// compare 	record_date_time with from_date_time
		...
		{
			search_index+=no_of_records_per_day;	// increase the search index by 96/48/24
			if((search_index-initial_search_index)>g_Class07_Blockload_EntriesInUse)
				search_index = initial_search_index + g_Class07_Blockload_EntriesInUse;
			addr=(int32_t)SURVEY_BASE_ADDRESS + (int32_t)(search_index%NO_OF_SURVEY_RECORDS)*(int32_t)SURVEY_PTR_INC;
			
		}
	}
	while ((done==0)&&(loop_count<NO_OF_DAYS_SURVEY_DATA));
...
// we will also exit the loop if we have searched all the data
	...
	if((done==0)&&(loop_count==NO_OF_DAYS_SURVEY_DATA))
...
//	if(match_result>=0)
	...
	// search_index is the index of that record
		...
		do
		{
			...
			{
			  decptr_maskless(&addr, SURVEY_BASE_ADDRESS, SURVEY_END_ADDRESS, SURVEY_PTR_INC);
...
// start from search_index and increase in steps of no_of_records_per_day till match_result>=0 or loop_count==NO_OF_DAYS_SURVEY_DATA 
...
// the remaining values cannot be copied straightaway - since the data types are different
	...
	g_Class07_EventBuffer.current_value_IR=eventdata.current_value_IR;
	g_Class07_EventBuffer.current_value_IY=eventdata.current_value_IY;
	g_Class07_EventBuffer.current_value_IB=eventdata.current_value_IB;
...
// It may be referrred to in Copy(5) or earlier - ajayjain
